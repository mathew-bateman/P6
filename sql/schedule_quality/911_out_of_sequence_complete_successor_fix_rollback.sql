/* Restore the original workbook rule: exclude only when both tasks are complete. */
SET NOCOUNT ON;
SET XACT_ABORT ON;

IF DB_NAME() <> N'P62212_1'
    THROW 51860, 'This rollback must be run against P62212_1.', 1;
GO
...
SELECT OBJECT_NAME(OBJECT_ID(N'[powerbitables].[xertoolkit_fn_out_of_sequence]')) AS restored_function;
