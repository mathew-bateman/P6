/* Restore the original zero-hour Critical Tasks boundary. */
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF OBJECT_ID(N'[powerbitables].[xertoolkit_fn_activity_quality]', N'IF') IS NULL
    THROW 51911, 'The schedule-quality activity function is missing.', 1;

DECLARE @definition nvarchar(max) =
    OBJECT_DEFINITION(OBJECT_ID(N'[powerbitables].[xertoolkit_fn_activity_quality]'));

IF @definition NOT LIKE N'%total_float_hr_cnt < 4.0%'
   OR @definition NOT LIKE N'%total_float_hr_cnt >= 4.0%'
    THROW 51912, 'The activity-quality function does not have the four-hour boundaries.', 1;
...
IF OBJECT_DEFINITION(OBJECT_ID(N'[powerbitables].[xertoolkit_fn_activity_quality]'))
   ...
   OR OBJECT_DEFINITION(OBJECT_ID(N'[powerbitables].[xertoolkit_fn_activity_quality]'))
       NOT LIKE N'%total_float_hr_cnt > 0%'
    THROW 51913, 'The zero-hour critical boundary was not restored.', 1;
